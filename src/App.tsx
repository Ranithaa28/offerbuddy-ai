...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (cachedId) {
      ...
      const cachedDomain = localStorage.getItem("offerbuddy_user_domain") || "Tech & IT";
  ...
  const renderTabContent = () => {
    if (!user) {
      ...
      return <LandingPage theme={theme} onChangeTheme={changeTheme} onNavigate={setCurrentTab} onStartAuth={() => setCurrentTab("auth")} />;
    }
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans selection:bg-blue-600 selection:text-white theme-${theme} bg-slate-950 text-slate-700 transition-colors duration-300`}>
      {!user ? (
