...
export const DOMAIN_PROBLEMS: { [domain: string]: Problem[] } = {
  "Tech & IT": [
    {
      title: "Two Sum",
      difficulty: "Easy",
  ...
  "Healthcare & BioSciences": [
    {
      title: "DNA Transcription & Translation Script",
      difficulty: "Easy",
      description: "Write a script that takes a DNA sequence string (e.g. 'ATGCGATCG') and transcribes it to mRNA (replacing T with U) and then translates it into Amino Acid letters (e.g. AUG -> Met, etc.).\nInput: 'ATGCGATCG'\nOutput: 'mRNA: AUGCGAUCG | Translation: Met-Arg-Ser'",
      boilerplates: {
        TypeScript: "function translateDNA(seq: string): string {\n  // Write bioinformatics transcription & translation logic here\n  return 'mRNA: ' + seq.replace(/T/g, 'U') + ' | Translation: Met-Arg-Ser';\n}",
        Python: "def translateDNA(seq: str) -> str:\n    # Write bioinformatics transcription & translation logic here\n    return 'mRNA: ' + seq.replace('T', 'U') + ' | Translation: Met-Arg-Ser'",
        JavaScript: "function translateDNA(seq) {\n  // Write bioinformatics transcription & translation logic here\n  return 'mRNA: ' + seq.replace(/T/g, 'U') + ' | Translation: Met-Arg-Ser';\n}"
      }
    },
    {
      title: "Protein Isoelectric Point (pI) Estimator",
      difficulty: "Medium",
      description: "Calculate the estimated net charge of a given peptide sequence at standard physiological pH 7.4. Use basic amino acid pKa values (Lys=10.0, Arg=12.5, His=6.0, Asp=3.9, Glu=4.2).\nInput: Peptide sequence 'KRDH'\nOutput: Estimated charge value.",
      boilerplates: {
        TypeScript: "function estimatePeptideCharge(seq: string, pH: number = 7.4): number {\n  // Write logic to calculate net charge based on basic and acidic residues\n  return 0.0;\n}",
        Python: "def estimatePeptideCharge(seq: str, pH: float = 7.4) -> float:\n    # Write logic to calculate net charge based on basic and acidic residues\n    return 0.0",
        JavaScript: "function estimatePeptideCharge(seq, pH = 7.4) {\n  // Write logic to calculate net charge based on basic and acidic residues\n  return 0.0;\n}"
      }
    }
  ],
  "Finance & Business": [
    {
      title: "Compound Interest & Mortgage Amortization",
      difficulty: "Easy",
      description: "Write an amortization function that calculates periodic interest and principal splits given an initial loan balance, annual interest rate, and term in months.\nInput: Balance=100000, Rate=0.06, Months=12\nOutput: Returns monthly breakdown of interest and principal paid.",
      boilerplates: {
        TypeScript: "function calculateAmortization(principal: number, annualRate: number, months: number): any[] {\n  // Return array of monthly interest/principal breakdowns\n  return [];\n}",
        Python: "def calculateAmortization(principal: float, annualRate: float, months: int) -> list:\n    # Return array of monthly interest/principal breakdowns\n    return []",
        JavaScript: "function calculateAmortization(principal, annualRate, months) {\n  // Return array of monthly interest/principal breakdowns\n  return [];\n}"
      }
    },
    {
      title: "Portfolio Value at Risk (VaR) Estimator",
      difficulty: "Medium",
      description: "Estimate the 95% confidence level Value at Risk (VaR) for a two-asset portfolio given daily return lists and asset weights.\nInput: AssetA returns [0.01, -0.02, 0.005...], AssetB returns [...], Weights [0.6, 0.4]\nOutput: VaR percentage estimate.",
      boilerplates: {
        TypeScript: "function estimatePortfolioVaR(returnsA: number[], returnsB: number[], weights: number[]): number {\n  // Write portfolio risk simulation or historical percentile logic\n  return 0.0;\n}",
        Python: "def estimatePortfolioVaR(returnsA: list, returnsB: list, weights: list) -> float:\n    # Write portfolio risk simulation or historical percentile logic\n    return 0.0"
      }
    }
  ],
  "Creative Arts & Design": [
    {
      ...
      difficulty: "Easy",
      description: "Write an accessibility checking script that calculates the relative luminance and contrast ratio between two hex colors (foreground and background) based on WCAG 2.1 compliance rules.\nInput: '#FFFFFF', '#000000'\nOutput: 'Contrast Ratio: 21:1 (AAA Compliant)'",
      boilerplates: {
        TypeScript: "function checkContrastRatio(hex1: string, hex2: string): string {\n  // Write relative luminance and contrast logic\n  return '21:1 (AAA Compliant)';\n}",
        Python: "def checkContrastRatio(hex1: str, hex2: str) -> str:\n    # Write relative luminance and contrast logic\n    return '21:1 (AAA Compliant)'"
      }
    },
    {
      title: "Responsive Typography & Spacing Scale Solver",
      difficulty: "Medium",
      description: "Calculate dynamic typography fluid scaling (using CSS clamp) given minimum/maximum viewport widths and minimum/maximum font sizes.\nInput: minViewport=320, maxViewport=1200, minSize=16, maxSize=24\nOutput: Clamp CSS string.",
      boilerplates: {
        TypeScript: "function generateFluidTypography(minVp: number, maxVp: number, minSize: number, maxSize: number): string {\n  // Solve linear scaling equations and output CSS clamp() standard string\n  return 'clamp(1rem, 2vw + 0.8rem, 1.5rem)';\n}",
        Python: "def generateFluidTypography(minVp: int, maxVp: int, minSize: int, maxSize: int) -> str:\n    # Solve linear scaling equations and output CSS clamp() standard string\n    return 'clamp(1rem, 2vw + 0.8rem, 1.5rem)'"
      }
    }
  ],
  "Core Engineering & Operations": [
    {
      title: "Stress-Strain Elasticity Solver",
      difficulty: "Easy",
      description: "Write a function to calculate stress, strain, and check if a given load exceeds the tensile yield strength of a cylindrical metal specimen under axial tension.\nInput: Force, Diameter, Original Length, Elongation, YieldStrength\nOutput: Object with calculated values and a 'SafetyStatus' string.",
      boilerplates: {
        TypeScript: "function solveElasticity(force: number, diameter: number, length: number, elongation: number, yieldStrength: number): any {\n  // Calculate and return stress, strain, and safety safety assessment\n  return {};\n}",
        Python: "def solveElasticity(force: float, diameter: float, length: float, elongation: float, yieldStrength: float) -> dict:\n    # Calculate and return stress, strain, and safety safety assessment\n    return {}"
      }
    },
    {
      title: "Proportional-Integral-Derivative (PID) Simulator",
      difficulty: "Medium",
      description: "Simulate a feedback loop PID controller's adjustment process to bring a process variable (e.g. temperature) to a target setpoint over N discrete time steps.\nInput: Setpoint, InitialValue, Kp, Ki, Kd, Steps\nOutput: Series of actuator outputs.",
      boilerplates: {
        TypeScript: "function simulatePID(setpoint: number, initVal: number, Kp: number, Ki: number, Kd: number, steps: number): number[] {\n  // Write control loop simulation feedback math\n  return [];\n}",
        Python: "def simulatePID(setpoint: float, initVal: float, Kp: float, Ki: float, Kd: float, steps: int) -> list:\n    # Write control loop simulation feedback math\n    return []"
      }
    }
  ]
};
...
export default function CodingEvaluationView({ userId, onActivityAdded }: CodingEvaluationViewProps) {
  // Determine domain and active problem sets dynamically
  const [domainSector, setDomainSector] = useState(() => localStorage.getItem("offerbuddy_user_domain") || "Tech & IT");
  
  const problems = DOMAIN_PROBLEMS[domainSector] || DOMAIN_PROBLEMS["Tech & IT"];
  ...
  const [code, setCode] = useState(problems[0].boilerplates["TypeScript"] || Object.values(problems[0].boilerplates)[0]);
        ...
        <div className="space-y-1.5 max-w-xl text-left">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 bg-blue-500/20 text-blue-300 text-[9px] font-extrabold rounded-full uppercase tracking-wider border border-blue-500/30">
              {domainSector === "Tech & IT" ? "SDE Placement Engine" : `${domainSector} Prep Engine`}
            </span>
            <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-emerald-400 text-[10px] font-mono">Process Evaluator V4</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight flex items-center gap-2">
            {domainSector === "Tech & IT" ? "AI Code Compiler & Challenge Hub" : "AI Logical Reasoning & Challenge Hub"} <Code2 className="h-5 w-5 text-blue-400" />
          </h1>
          <p className="text-slate-300 text-[11px] sm:text-xs font-sans leading-relaxed">
            {domainSector === "Tech & IT" 
              ...
              : `Solve practical reasoning and data analysis tasks for ${domainSector}. Receive instant domain expert evaluations and unlock **Advanced Titan Stories**!`}
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl w-full md:w-auto text-left justify-between sm:justify-start">
          <div className="space-y-1">
            ...
            <span className="text-[8px] text-slate-400 block mt-0.5 font-mono">
              {bonusPoints >= 500 ? "🎉 Tier 3 Stipend Bonus Active!" : `Need ${Math.max(0, 500 - bonusPoints)} pts for free ${domainSector === "Tech & IT" ? "SDE" : "Domain"} Certificate`}
            </span>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 space-y-4 shadow-sm text-left">
            <h3 className="font-bold text-slate-800 text-xs uppercase tracking-wider">
              {domainSector === "Tech & IT" ? "Select DSA Challenge" : `Select ${domainSector} Challenge`}
            </h3>
