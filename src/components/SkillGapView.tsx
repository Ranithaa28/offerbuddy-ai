...
import { SECTOR_ROLES } from "./ProfileView";
...
export default function SkillGapView({ userId, onActivityAdded }: SkillGapViewProps) {
  ...
  const presets = SECTOR_ROLES[domainSector] || SECTOR_ROLES["Tech & IT"];

  const [targetRole, setTargetRole] = useState(() => localStorage.getItem("offerbuddy_user_role") || presets[0]);
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 text-xs sm:text-sm">
          Map your background against high-paying {domainSector} roles. Receive automated skill gaps, 4-week roadmap schedules, and suggested projects.
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 space-y-4 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-2">
          <input
            ...
            placeholder={`Or write custom role, e.g. ${presets[0] || "Specialist"}`}
